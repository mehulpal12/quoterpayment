 ...
 const [features, setFeatures] = useState<Feature[]>([]);
  const [posts, setPosts] = useState<any[]>([]);
    ...
    async function getData() {
      const featureQuery = `*[_type == "feature"]{id, name, price, description}`;
      const postQuery = `*[_type == "post"]{name, price, publishedAt, slug}`;
      const [fetchedFeatures, fetchedPosts] = await Promise.all([
        client.fetch(featureQuery),
...
//   const features = [
//     // set a static name for the blogging feature; posts are rendered below when selected
//     { id: "blogging", name: "Blogging ", price: 3000 },
  ...
  <div className="">
          ...
          <ul className="">
            {posts.map((post) => {
              ...
              return (
                <li key={slug || title} className="p-3 bg-[#0b1220] rounded">
                  <div className="text-slate-100 font-semibold">{title}{price}</div>
                  {slug && <div className="text-slate-400 text-sm">Slug: {slug}</div>}
                </li>
              );
            })}
          </ul>
